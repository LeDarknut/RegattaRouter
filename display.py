	...
	for x in range(0, wst.w):
		...
		for y in range(0, wst.h):
			...
			if np.isnan(wst.table[0][x][y][0]):
				...
				if ((x <= 0         or np.isnan(wst.table[0][x - 1][y][0])) and
					(x >= wst.w - 1 or np.isnan(wst.table[0][x + 1][y][0])) and
					(y <= 0         or np.isnan(wst.table[0][x][y - 1][0])) and 
					(y >= wst.h - 1 or np.isnan(wst.table[0][x][y + 1][0]))):
	...
	for x in range(0, ws.w):
		...
		for y in range(0, ws.h):
			...
			if np.isnan(ws.table[x][y][0]):
				...
				if ((x <= 0         or np.isnan(ws.table[x - 1][y][0])) and
                    (x >= wst.w - 1 or np.isnan(ws.table[x + 1][y][0])) and
                    (y <= 0         or np.isnan(ws.table[x][y - 1][0])) and 
                    (y >= wst.h - 1 or np.isnan(ws.table[x][y + 1][0]))):
