...
def divide(name, T0, T1, W, E, S, N) :
	...
	if not(os.path.exists("wind/region/" + name)) :

		os.mkdir("wind/region/" + name)

	for f in files :
			
		os.popen("wgrib2 wind/global/{0}.grb -small_grib {1}:{2} {3}:{4} wind/region/{5}/{0}.grb".format(f, W, E, S, N, name)).read()
